...
class QuantizedESPCN(nn.Module):
    def __init__(
        ...
        if self.method in ("none", "adaround"):
            # AdaRound: weights are already rounded; no runtime quant
            identity = lambda x: x
            self.quant_conv1 = QuantizedConv2d(conv1, identity, identity)
            self.quant_conv2 = QuantizedConv2d(conv2, identity, identity)
            self.quant_conv3 = QuantizedConv2d(conv3, identity, None)
        else:
            # QAT methods: MUST create real quantizer modules to load state_dict
            strategy_cls = QUANTIZER_MAP[self.method]
            strategy = strategy_cls(self.quant_config)

            # Create real quantizer modules (not lambdas!)
            wq1 = strategy.create_weight_quantizer("conv1", conv1)
            aq1 = strategy.create_activation_quantizer("conv1", conv1)
            self.quant_conv1 = QuantizedConv2d(conv1, wq1, aq1)

            wq2 = strategy.create_weight_quantizer("conv2", conv2)
            aq2 = strategy.create_activation_quantizer("conv2", conv2)
            self.quant_conv2 = QuantizedConv2d(conv2, wq2, aq2)

            wq3 = strategy.create_weight_quantizer("conv3", conv3)
            self.quant_conv3 = QuantizedConv2d(conv3, wq3, None)
